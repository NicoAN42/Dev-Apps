...
def process_folders():
    ...
    for idx, branch in enumerate(branches):
        ...
        if found:
            preview_box.insert("end", f"  Injected data:\n", "info")
            ...
            preview_box.insert("end", f"  ✗ Skipped: No valid Excel files\n\n", "warning")
...
tb.Label(app, text="Utilization Report Generator", font=("Helvetica", 18, "bold")).pack(pady=20)

frm = tb.Frame(app, padding=10)
frm.pack(fill="x")

tb.Label(frm, text="Report Maker Name:").grid(row=0, column=0, sticky="w")
name_entry = tb.Entry(frm, width=40)
name_entry.grid(row=0, column=1, padx=10)

tb.Label(frm, text="Select Parent Folder:").grid(row=1, column=0, sticky="w", pady=10)
folder_path = tb.StringVar()
tb.Entry(frm, textvariable=folder_path, width=40).grid(row=1, column=1)
tb.Button(frm, text="Browse", command=lambda: folder_path.set(filedialog.askdirectory())).grid(row=1, column=2)

tb.Label(frm, text="Upload Word Template (.docx):").grid(row=2, column=0, sticky="w", pady=10)
word_path = tb.StringVar()
tb.Entry(frm, textvariable=word_path, width=40).grid(row=2, column=1)
